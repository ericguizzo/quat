...
#SBATCH --job-name=quat_2                 # Job name
...
#SBATCH --ntasks-per-node=6                 # Use all the cores on each node
#SBATCH --mem=60G                              # Expected memory usage (0 means use all available memory)
#SBATCH --time=72:00:00                      # Time limit hrs:min:sec
#SBATCH --output=experiment_quat_2_%j.out        # Standard output and error log [%j is replaced with the jobid]
#SBATCH --error=experiment_quat_2_%j.error
#SBATCH --gres=gpu:1
