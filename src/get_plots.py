...
def gen_plot(o, r, v, a, d, sound_id, curr_path, format='png'):
    ...
    plt.subplot(231)
    plt.title('Original')
    plt.pcolormesh(np.flip(o.T,-1)/np.max(o)[:,:,-1])
    plt.suptitle('AUTOENCODER OUTPUT MATRICES')
    plt.subplot(232)
    plt.title('Real')
    plt.pcolormesh(np.flip(r.T,-1)/np.max(r)[:,:,-1])
    plt.subplot(233)
    plt.title('Valence')
    plt.pcolormesh(np.flip(v.T,-1)/np.max(v)[:,:,-1])
    plt.subplot(234)
    plt.title('Arousal')
    plt.pcolormesh(np.flip(a.T,-1)/np.max(a)[:,:,-1])
    plt.subplot(235)
    plt.title('Dominance')
    plt.pcolormesh(np.flip(d.T,-1)/np.max(d)[:,:,-1])
