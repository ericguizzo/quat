...
parser.add_argument('--load_custom_matrices', type=str, default='False')
parser.add_argument('--training_predictors', type=str, default=None)
parser.add_argument('--training_target', type=str, default=None)
parser.add_argument('--validation_predictors', type=str, default=None)
parser.add_argument('--validation_target', type=str, default=None)
parser.add_argument('--test_predictors', type=str, default=None)
parser.add_argument('--test_target', type=str, default=None)

#zeropad if desired:
parser.add_argument('--time_dyn_pad', type=str, default="False")
parser.add_argument('--time_dim', type=int, default=512)
parser.add_argument('--source_time_dim', type=int, default=512)  #just for dyn pad
parser.add_argument('--target_time_dim', type=int, default=512)  #just for dyn pad
...
parser.add_argument('--onehot_truth', type=str, default='False')
...
def evaluate(model, device, loss_function, dataloader, emo_weight):
    ...
    with tqdm(total=len(dataloader)) as pbar, torch.no_grad():
        ...
        for i, (sounds, truth) in enumerate(dataloader):
            ...
            if args.time_dyn_pad:
                sounds = uf.dyn_pad(sounds, device, args.source_time_dim, args.target_time_dim)
            if args.onehot_truth:
                truth = torch.nn.functional.one_hot(truth.to(torch.int64), args.model_num_classes)
